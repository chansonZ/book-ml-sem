...
_user_src_path = '/mnt/src'
_user_resource_path = '/mnt/resource' # 存放模型等资源文件的路径
_user_log_path = '/mnt/log'
...
# 单条数据格式
JSON_KEY_LIST=['subtype','key','data']
PACKAGE_KEY='package'
'''
{
    "subtype":"ios",
...
def check_single_args(web_args):
    if web_args is None or len(web_args)==0:
        return ErrCode.NO_PARA.value,'nothing get from http request!'

    code,msg = check_package_valid(web_args)
    if code != ErrCode.SUCCESS.value:
        return ErrCode.JSON_ERR.value,msg
...
def predict_single(args):
    ...
    if not isinstance(d,dict):
        ...
        return format_return(key,subtype,ErrCode.USER_ERR1.value,msg,result='')
    ...
    if g_models.models[subtype] is None:
        ...
        if set(['code','result','msg'] != set(d.keys())):
            return format_return(key,subtype,ErrCode.USER_ERR1.value,_bad_msg,result='')
        else:
            return format_return(key,subtype,d['code'],d['msg'],result=d['result'])
    else:
        code, msg = check_processed_data(d,subtype)
        if code != ErrCode.SUCCESS.value:
            return format_return(key,subtype,ErrCode.USER_ERR2.value,'After data_process():'+msg,result='')
        d = data_order(d,g_models.data_order[subtype])

        #这里只使用了 predict_proba
